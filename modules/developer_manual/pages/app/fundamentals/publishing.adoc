...
[IMPORTANT]
====
Apps uploaded to the Marketplace must carry a valid code signature. Starting with
ownCloud 11, app signing is mandatory: an unsigned or invalidly-signed
app is blocked on install, update, or enable. Sign your app with `ocsign` before
uploading — see xref:developer_manual:app/advanced/code_signing.adoc[Code Signing].
====
...
| summary
| Provide a short description (max. 90 chars). This gets displayed below the product title and on the product tiles; mandatory since ownCloud Classic 10.0.0.
