...
The first thing to do is to ensure that your server has the necessary software for installing and running ownCloud. While you can go further, you need to proceed with xref:admin_manual:installation/installing_with_docker.adoc[Installing With Docker]. Then, you will need to install the software required to run the development environment's installation process.
...
These dependencies need to be installed manually creating an own Docker Image.
...
* Clone the development version from https://github.com/owncloud/core[GitHub]:
...
There are a few ways to identify the user and group the webserver is running as. Likely the easiest are `grep` and `ps`. Here’s an example of using both (which assumes that the distribution is Ubuntu 24.04).
...
With the ownCloud source installed locally, next install ownCloud’s dependencies by running https://www.gnu.org/software/make/[Make], from the directory where ownCloud’s located. Here’s an example of how to do so:
...
With all that done, install ownCloud using the xref:admin_manual:installation/installing_with_docker.adoc[Installing With Docker] documentation.
