...
The password is an optional parameter and you will be asked for it if you didn't provide it as in the example above. If necessary, the workgroup can be set in the username as well. Use the syntax `'<workgroup>\<username>'` for this. It is important to use quotes to keep the backslash. The whole example command looks like:
