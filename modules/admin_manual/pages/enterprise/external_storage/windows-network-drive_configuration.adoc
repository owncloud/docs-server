...
[NOTE]
====
* This command needs configuration in the docker compose environment to be activated. A prepared example is given but needs configuration. You need to create as many services as you need listeners.
...
As aa example, you can issue the following command, replacing <host> and <share> accordingly:
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} wnd:listen host share username password
