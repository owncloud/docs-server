...
It is advised to redirect messages into a separate file. To do so, add these lines to `config.php` and adjust the target path accordingly. Otherwise please make sure to adjust the server log level to 1 (or 0), otherwise the Auditing app won't log anything. Configuring a seperate file circumvents the global log level. Note that the target path must be writeable for the web server user:
