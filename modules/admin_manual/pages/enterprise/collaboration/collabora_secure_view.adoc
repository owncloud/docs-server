...
Note that in this case, we need to also remove entries in config.apps.sample (richdocuments) and do a config-to-docs run.
...
When Collabora Online is properly set up and integrated into ownCloud Classic, secure view functionality is available. Secure view is a mode where users can place limitations on files and folders that are shared.
