...
The Workflow App should be enabled by default (Apps page), and the three configuration modules will be visible on your ownCloud Admin page. See xref:classic_ui:files/webgui/tagging.adoc[Tagging Files] in the ownCloud User manual to learn how to apply and filter tags on files.
...
image:enterprise/file_management/workflow-1.png[Tag Manager, width=250]
...
image:enterprise/file_management/workflow-5.png[Tag Manager example, width=250]
...
image:enterprise/file_management/workflow-2.png[Automatic tagging, width=350]

When files with this tag are shared with you, you can view them with the Tags filter on the Files page.

image:enterprise/file_management/workflow-3.png[Viewing tagged files, width=250]
...
image:enterprise/file_management/update_workflow.png[Update Workflow, width=350]
...
image:enterprise/file_management/workflow-4.png[Setting retention times via tag, width=350]
...
You can define the way that the retention engine behaves by adding the following environment variable. The value can be either 
`tagbased` (default) or `userbased`.

[source,.env]
----
OWNCLOUD_WORKFLOW_RETENTION_ENGINE=userbased
----
--
