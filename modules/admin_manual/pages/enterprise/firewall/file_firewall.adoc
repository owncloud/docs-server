...
image:enterprise/firewall/firewall-1.png[Figure 1: Empty File Firewall configuration panel, width=350]
...
image:enterprise/firewall/firewall-2.png[Figure 2: Two example rules that restrict logins per user group, width=350]
...
CAUTION: Rules are saved into the configuration file. We do not recommend manipulating it. These rules use JSON encoding, so the values are difficult to read and a single typo will break all of your rules.

=== Controlling Access to Folders

The easiest way to block access to a folder, is to use a system tag. A new rule type was added which allows you to block access to files and folders, where at least one of the parents has a given tag.
