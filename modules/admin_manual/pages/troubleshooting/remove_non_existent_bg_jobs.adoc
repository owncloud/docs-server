...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} background:queue:delete ID
----
...
If an erroring job as been added manually in `crontab`, remove the job from the crontab queue list on your host. First, list the entries of crontab for the web server user:
