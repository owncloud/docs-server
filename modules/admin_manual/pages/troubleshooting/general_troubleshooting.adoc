...
* To find any issues you need to araise the log level to `All` or to `Everything` via the `OWNCLOUD_LOG_LEVEL` environment variable or via your ownCloud Admin page. Please see xref:configuration/server/logging/logging_configuration.adoc[Logging Configuration] for more information on these log levels.
...
Please open an issue in the https://github.com/owncloud/core/issues[ownCloud Classic repo].

== Troubleshooting Contacts & Calendar

=== Service Discovery

Some clients - especially on iOS or macOS - may have problems finding the proper sync URL, even when explicitly configured to use it.

If problems arise while using CalDAV or CardDAV clients together with ownCloud, open an issue in the https://github.com/owncloud/core/issues[ownCloud Classic repo].
