...
* Details for the occ command can be found in the xref:configuration/server/occ_command.adoc#migrate-to-ocis[Migrate to oCIS] occ command reference.
...
Because data migration requires steps to be passed, even nothing needs to be done, the following commands must be issued:

Mark user and groupe migration as dome::
+
--
[source,bash]
----
sudo -u www-data \
  php /var/www/owncloud/occ \
  migrate:to-ocis:migrate:users --skip admin
----
...
sudo -u www-data \
  php /var/www/owncloud/occ \
  migrate:to-ocis:migrate:groups --skip admin
----
--

== Preparing Data Migration

After the user, group and role prerequisites are met, the migration can be started.
