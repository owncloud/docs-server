...
If you want to use SSL certificates with MyQnapCloud as a DDNS service, add a reverse proxy setup as described in this articla :
https://www.qnap.com/en/how-to/tutorial/article/how-to-use-reverse-proxy-to-improve-secure-remote-connections
Add a reverse proxy from your myqnapcloud domain with an optional as source and `http://localhost:4490` as destination.
Make sure to change the OwnCloud config to use your myqnapcloud domain.
