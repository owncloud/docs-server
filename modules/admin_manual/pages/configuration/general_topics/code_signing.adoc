...
image:issues/code-integrity-notification.png[Code integrity warning banner., width=350]
...
image:issues/code-integrity-admin.png[Links for resolving code integrity warnings., width=350]
...
. Download the official server tar ball and copy the `.htaccess` into your instance.

For other means on how to receive support please take a look at he {oc-help-url}[Docs & Guides page]. After fixing these problems verify by clicking btn:[Rescan].
...
{occ-command-example-prefix-docker} integrity:check-app <appid>
----
