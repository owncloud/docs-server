...
. A fully functioning {elastic-search-url}[Elasticsearch Server]. Follow the {elastic-search-install-url}[Installation and Upgrade Guide] for your environment.
+
[NOTE]
====
With version >=2.0.0 of the app:

* Elasticsearch version > 7 is required and has been tested
* and is supported up to version 8.6.2 though newer versions may work without any issues.
====

. The {ingest-url}[Ingest Attachment Processor Plugin] lets Elasticsearch extract metadata and text from over a thousand different file types such as PPT, XLS, PDF and more. To install the processor, run the following command _from your Elasticsearch installation directory_:
+
[source,bash]
----
...
----
{occ-command-example-prefix-docker} app:enable search_elastic
----
...
image::apps/search_elastic/search_elastic_auth_items.png[Auth Items,width=350]
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} search:index:reset
----
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} config:list search_elastic
----
+
[source,plaintext]
----
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} search:index:create
----
...
image::apps/search_elastic/warning_could_not_setup_indexes_connection_failure.png[Warning no Index,width=300]

image::apps/search_elastic/warning_could_not_setup_indexes_unknown_key.png[Warning unknown Key,width=300]
