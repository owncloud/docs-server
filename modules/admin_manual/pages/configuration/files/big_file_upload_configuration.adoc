...
* Your temp file or partition has to be big enough to hold multiple parallel uploads from multiple users. The formula for this is `temp_space = concurrent_uploads * chunk size` +
...
+
----
10 MB x 25 users = 250MB required temp space
----
