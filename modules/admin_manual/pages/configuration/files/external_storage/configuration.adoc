...
+
NOTE: The command xref:configuration/server/occ_command.adoc#the-filesscan-command[occ files:scan] respectively any xref:configuration/server/occ_command.adoc#managing-background-jobs[background job] containing _files scan_ is only available for POSIX but not for S3 and compatible filesystems. See the  for more information.
