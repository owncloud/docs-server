...
* If you are running an encryption module before version 1.5.0 or started with a version before 1.5.0 BASE64 encyption was used and might still be used for old files if untouched since then. In these cases old encrypted files increased their size by roughly 35%. We recommend to update the system.
