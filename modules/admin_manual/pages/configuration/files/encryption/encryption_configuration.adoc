...
NOTE: Using Docker means that the data directory is a subfolder of the volumes mount point, which is required for certain file configurations. For more details see the xref:installation/mount_folder_structure.adoc[Mount Folder Structure] documentation.
...
NOTE: Starting with ownCloud 9.0 we support Authenticated Encryption for all newly encrypted files. See {hackerone-url}[Exploiting unauthenticated encryption mode] for more technical information about the impact.
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} maintenance:singleuser --on
----
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} encryption:encrypt-all --yes
----
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} maintenance:singleuser --off
----
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} encryption:decrypt-all [username]
----
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} encryption:disable
----
...
.  Edit your database table `oc_appconfig` and remove the rows with the config keys `recoveryKeyId` and `recoveryAdminEnabled` for the appid `files_encryption`.
...
[source,bash]
----
mkdir /mount_point/files/new_keys
----

[source,bash]
----
chown -R root:www-data /mount_point/files/new_keys
----

[source,bash]
----
chmod -R 0770 /mount_point/files/new_keys
----

[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} encryption:change-key-storage-root new_keys
----
