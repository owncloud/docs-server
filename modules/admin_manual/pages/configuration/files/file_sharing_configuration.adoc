...
image:configuration/files/sharing-files-settings.png[ownCloud Sharing settings, width=350]
...
mail notifications for shared files in ownCloud., width=350]
...
[NOTE]
====
* In the example above, the `<language code>` is an {two-letter-url}[ISO 3166-1 alpha-2 two-letter country code], such as *de*, *gb*, *us*, *es* or others.
...
image:configuration/files/sharing/sharing-files-via-social-media.png[ownCloud social media sharing links, width=350]
...
=== Set Default Expiration Ddate for Rremote Shares
...
NOTE: This setting does not apply to the Federated Cloud sharing feature. If xref:configuration/files/federated_cloud_sharing_configuration.adoc[Federated Cloud Sharing] is enabled, users can still share items with any users on any instances (_including the one they are on_) via a remote share.
...
image:configuration/files/sharing/blacklisting-groups.png[Blacklisting groups, width=350]
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} files:troubleshoot-transfer-ownership --uid <uid>`
----
====
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} files_external:create \
...
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix-docker} files_external:import \
    /import.json
----
