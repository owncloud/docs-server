...
. Run `system:cron` in the ownCloud directory of both servers (for example `/var/www/owncloud/`)
+
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix} system:cron
----
. Run the `OCA\Federation\SyncJob` job with the `force` option (you can get the correspondent job ID by using the `background:queue:status` occ command)
+
[source,bash,subs="attributes+"]
----
{occ-command-example-prefix} background:queue:execute --force 10
----
