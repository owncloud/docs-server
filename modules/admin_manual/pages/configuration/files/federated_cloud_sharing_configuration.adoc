...
----
. Run the `OCA\Federation\SyncJob` job with the `force` option on both servers (you can get the correspondent job ID by using the `background:queue:status` occ command)
+
[source,bash,subs="attributes+"]
----
