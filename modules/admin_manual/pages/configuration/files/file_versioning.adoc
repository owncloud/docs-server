...
WARNING: Note that retention policy can be applied after restore operation, which could result in the old version be removed via retention policy. This behaviour can be changed with persistent major version discussed in the section below.

== Extended Version Metadata

 You can enable and disable below features via an entry in config.php. For details see: xref:configuration/server/config_sample_php_parameters.adoc#FIXME[Save extended version metadata of uploaded and edited files]. 

image::configuration/files/files_versions/version-metadata.png[Extended Version Metadata,width=250]

=== Show the Author of a Version

You can enable and disable to show the author of a version in the versions history. This is helpful to keep track of who changed a version. 
...
TIP: The versions follow the X.Y format, where X is a major part and Y is a minor part of the version. Version tags cannot be modified by the user and auto-generated by ownCloud. 
