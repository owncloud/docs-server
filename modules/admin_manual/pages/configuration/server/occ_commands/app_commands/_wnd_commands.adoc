...
Integrate Windows and Samba/CIFS shared network drives as external storages. For details please see the xref:enterprise/external_storage/windows-network-drive_configuration.adoc[Windows Network Drive (WND)] documentation.
...
----
{occ-command-example-prefix-docker} wnd:listen [options] [--] <host> <share> <username> [<password>]
----
...
[source,docker,subs="attributes+"]
----
{occ-command-example-prefix-docker} wnd:process-queue [options] [--] <host> <share>
----
...
[source,docker,subs="attributes+"]
----
{occ-command-example-prefix-docker} wnd:set-service-account [options] [--] <mount-id>
----
