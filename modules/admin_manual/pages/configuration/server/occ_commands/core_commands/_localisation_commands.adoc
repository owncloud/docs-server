...
First, create a source translation file in `/var/www/owncloud/apps/comments/l10n`, called `de_AT.php`. In it, add the required translation strings, as in the following example. Refer to the developer documentation on xref:developer_manual:app/advanced/l10n.adoc#creating-your-own-translatable-files[creating translation files], if you're not familiar with creating them.
...
----
{occ-command-example-prefix-docker} l10n:createjs comments de_AT
----
...
[source,docker,subs="attributes+"]
----
{occ-command-example-prefix-docker} l10n:createjs comments de_AT de_DE hu_HU es fr
----
