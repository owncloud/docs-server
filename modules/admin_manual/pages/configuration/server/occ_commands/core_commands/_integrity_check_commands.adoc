...
integrity
 ...
 integrity:sign-app                  Signs an app using a private key. (deprecated)
 integrity:sign-core                 Sign core using a private key. (deprecated)
----
...
[IMPORTANT]
====
`integrity:sign-app` and `integrity:sign-core` are *deprecated*. Signing is now done with the standalone `ocsign` tool, which does not require a running ownCloud instance. See xref:developer_manual:app/advanced/code_signing.adoc[Code Signing] in the Developer Manual for the current signing workflow.
====
