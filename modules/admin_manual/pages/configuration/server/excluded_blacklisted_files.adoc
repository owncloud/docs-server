...
For example configurations please see the xref:configuration/server/config_sample_php_parameters.adoc[config.sample.php] file.

Use the environment variables: `OWNCLOUD_BLACKLISTED_FILES` and `OWNCLOUD_EXCLUDED_DIRECTORIES` to configure them.
...
.. There is no common naming convention for these directories, and there most likely will never be. For example, NetApp uses `.snapshot` and `~snapshot`, EMC e.g., `.ckpt`, HDS e.g., `.latest` and `~latest`, and the ZFS filesystem uses `.zfs`
...
Example `excluded_directories` entries in `OWNCLOUD_EXCLUDED_DIRECTORIES` can look like this:

[source,.env]
----
OWNCLOUD_EXCLUDED_DIRECTORIES='.snapshot','~snapshot','dir1','dir2'
----
...
[source,.env]
----
OWNCLOUD_BLACKLISTED_FILES='hosts','evil_script.sh'
----
