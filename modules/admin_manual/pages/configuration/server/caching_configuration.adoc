...
The easiest cache to use is APCu, because it is a pure data cache, very fast as it is memory based only and _nothing needs to be configured_. APCu can not be used, when needed to run on an external server like in a distributed environment.
...
* If you do not have a distributed environment where the Redis server is installed at a different server, you can configure Redis to be xref:redis-configuration-using-unix-sockets[socket based] instead TCP based to xref:redis-performance-comparison-tcp-vs-socket[increase performance].

==== Additional notes for Redis vs. APCu on Memory Caching

* APCu is faster at local caching than Redis when using TCP, as APCu is memory based only.
...
Regardless of whether you have setup Redis to use TCP or a Unix socket, we recommend adding the following for best performance. This enables xref:configure-transactional-file-locking[External Transactional File Locking] based on Redis:
...
. Open an editor of your choice and change/add the following parameters to the redis config. Note that in the example, `vi` is used as editor:
+
[source,bash]
----
...
Then, configure ownCloud to communicate with Redis socket as in the following example. 
