...
+
To make the endpoint available under the static service discovery path, it is recommended to put a `RewriteRule` in place using in the `VirtualHost` section, the Apache modules `proxy` and `proxy_http` and `proxy_connect` (if SSL is used) have to be enabled:
+
[source,apache]
----
RewriteEngine on
