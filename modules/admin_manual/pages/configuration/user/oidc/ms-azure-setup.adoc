...
To allow the ownCloud clients (ownCloud Web/desktop/Android/iOS) to interact with the ownCloud Server, you have to install and enable to oAuth2 app please go to https://doc.owncloud.com/server/next/admin_manual/configuration/server/security/oauth2.html for details. In this setup your ownCloud Clients will work without extra hassles, but please be advised that the ownCloud Server is acting as identity provider for the clients and you will not be able to delete those clients directly from your indentity management solution.

=== Register ownCloud Desktop and Mobile Clients with Azure AD

To allow the ownCloud clients (ownCloud Web/desktop/Android/iOS) to interact directly with the identity provider, you have to register them as clients.
