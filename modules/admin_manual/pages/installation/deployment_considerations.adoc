...
ownCloud Server 11 runs exclusively in Docker containers. The host operating system only needs to provide Docker Engine and Docker Compose. The web server, PHP runtime, and application dependencies are all encapsulated within the ownCloud Docker images.
