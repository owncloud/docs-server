...
* **Local cache** (`memcache.local`) — defaults to APCu (`OWNCLOUD_MEMCACHE_LOCAL`). APCu is per-process and per-node. This is correct: it should *not* be shared.
* **Distributed cache** (`memcache.distributed`) and **locking cache** (`memcache.locking`) — must point at the shared Redis (set automatically with `OWNCLOUD_REDIS_ENABLED=true`).
...
When the Redis session handler is active, the image also exposes the PHP redis-session locking parameters: `OWNCLOUD_REDIS_SESSION_LOCKING_ENABLED` (default `1`), `OWNCLOUD_REDIS_SESSION_LOCK_WAIT_TIME` (default `20000`), and `OWNCLOUD_REDIS_SESSION_LOCK_RETRIES` (default `750`).
...
* **Database** — clustered MySQL/MariaDB (InnoDB) or PostgreSQL.
* **Redis** — single shared instance for locking, distributed cache, and sessions.
* **Storage** — the NFS export mounted at `OWNCLOUD_VOLUME_FILES` (consistent UID 33, `no_root_squash` or pre-owned), or S3 primary object storage.
