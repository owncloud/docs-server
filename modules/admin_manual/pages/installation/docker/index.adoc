...
The image contains the PHP and Apache setups, and nothing else needs to be added for it to work properly.
...
IMPORTANT: If you, for example, uninstall apps that are shipped with the image, these apps will be reappear on next container startup.
--
...
Here, you are able to run any command referring to xref:configuration/server/occ_command.adoc[Using the occ Command] by entering:
...
If, for any reason, you need to enter the owncloud containers CLI, you can do that with the following command:
