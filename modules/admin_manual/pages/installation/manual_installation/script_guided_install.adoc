...
:description: The purpose of the scripts provided are to ease any installation or upgrades of ownCloud instances. Using these scripts, you can also reapply ownership and access rights on linked directories if needed.
...
Installation::
When you do a fresh installation using this set of scripts, consider using links for the `data/` and `apps-external/` directories. Any copying or moving big directories can cause issues which can be avoided by using links. When doing an upgrade post of an installation, the scripts can relink those directories but will not "migrate" standard ones. If standard directories are used, the admin has to select a method to migrate those directories as part of the upgrade process which fits best to his environment. You will get notified about this task in case.

Migrating from standard to linked directories::
You can at any time relocate your `data/` and `apps-external/` directories and link them to the old location. This does not impact the functionality but eases upgrade steps a lot. In case, reparametrize the instance script with proper values to be prepared for the next upgrade. See some details at the xref:configuration[configuration section] below.

Upgrading::
When upgrading, the scripts rename the current ownCloud directory from the former name, usually `owncloud`, to `owncloud-timestamp`. A new and fresh owncloud directory will be created where the files from the tar archive get extracted to. By doing so, nothing gets deleted, overwritten or moved and the existing instance is secured to go back in case it is needed. Note that the script does not take care on the database. You have to manage a xref:maintenance/backup_and_restore/backup.adoc#backup-the-database[database backup] by yourself. When you have decided to use links for `data/` and `apps-external/`, the script will relink these directories. Nothing gets moved. You can also (re)run the scrips on linked directories to set correct ownership and permissions. When using standard directories, you have to manually migrate them according your environment and also manually check for correct ownership and permissions.
+
The scripts takes care on your configuration. The relevant files are _copied_ from the backup.
+
If the upgrade was successful, you can delete the backup or keep it based on your requirements.

Scripts::
There are two scripts available. The instance script defines the environment for your ownCloud while the preparation script called by the instance script does the main tasks. Note that the preparation script asks a couple of questions to decide what you want to achieve.
+
...
As you can see, and while this makes no difference on installing, upgrading is much easier as you do not need to take care on migrating directories as they just get relinked.
...
NOTE: In case of upgrading, if you have used links for the apps-external/ and data/ directory, the script will relink these directories. If you have used standard directories, you manually have to migrate any content of these directories back after the script has run. Check that ownership and permissions are correct after migration and in doubt, let the scrips set the correct ones when using linked directories or ste them manually - which can take a while in any case depending on the size of the data/ directory. Note you have take care not to overwrite new app versions with older ones.
...
.When upgrading
Follow the steps printed on the screen, especially when using standard directories where you have to migrate the `data/` and `apps-external/` directories manually befor finalizing the upgrade. Do not forget to reapply manually made changes made to `.htaccess` and `.user.ini` in your owncloud root directory.
