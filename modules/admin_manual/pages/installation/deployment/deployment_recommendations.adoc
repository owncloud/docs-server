...
:toc: right
:page-aliases: installation/deployment_recommendations.adoc
...
* Docker Engine and Docker Compose on the host. See xref:installation/system_requirements.adoc[System Requirements].
...
Any 64-bit Linux distribution capable of running Docker Engine 20.10 or later is supported.

==== SSL Configuration

SSL termination is handled by a reverse proxy in front of the Docker deployment. A standard SSL certificate is required.
...
* See {owncloud-edition-url}[ownCloud Classic or Enterprise Edition] for comparisons of the ownCloud editions.
...
Any 64-bit Linux distribution capable of running Docker Engine 20.10 or later is supported on each application server node.

==== SSL Configuration

SSL termination is done in the {haproxy-load-balancer-url}[HAProxy load balancer]. A standard SSL certificate is needed, installed according to the {haproxy-documentation-url}[HAProxy documentation].
