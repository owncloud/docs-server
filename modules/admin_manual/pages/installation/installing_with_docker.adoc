...
Here you need a bind mount and create an addifional config file that needs to be placed in the `<mount-point>/config` directory.
...
The Docker image comes with all apps provided by ownCloud, regardless of the licensing scheme. To see a list of the apps that have been shipped, you can either look in the GUI or list the contents of `/var/www/owncloud/apps` when accessing the xref:#container-cli[container CLI].
