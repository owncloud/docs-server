...
Location: `data/<user>/files`, see the: xref:server:admin_manual:configuration/server/occ_command.adoc#encryption[occ encryption command set]
...
NOTE: An admin can encrypt existing files post enabling encryption via an xref:server:admin_manual:configuration/server/occ_command.adoc#encryption[occ encryption command].
